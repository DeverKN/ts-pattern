import { Bind, MatchBind, PredicateBind, RestBind, SymbolForBind } from "../types/bind";
...
const isRestBind = <TBind extends RestBind<string, unknown> = RestBind<string, unknown>>(arg: unknown | TBind): arg is TBind => {
  return isBind(arg) && (arg.bindType === "matchRestBind" || arg.bindType === "predicateRestBind");
};

type BindOrRestBind = Bind<string, unknown> | RestBind<string, unknown>;
const isBind = <TBind extends BindOrRestBind = BindOrRestBind>(arg: unknown | TBind): arg is TBind => {
  return arg !== null && typeof arg === "object" && hasOwnProperty(arg, SymbolForBind);
};
...
const matchArray = <T, TArr extends T[]>(arr: TArr, pattern: ArrayPattern<TArr>): MatchResult => {
  ...
  if (isRestBind(first)) {
    return matchStartingRestBind(arr, first, rest);
  } else {
